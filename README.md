...
本 `custom` 分支在保留上游全部功能的基础上，做了以下精细化调整：
...
`light/` 和 `dark/` 目录包含已编译的 `.yaml` 文件，可直接使用无需运行 jsonnet。适合：

- 🚀 在仓/元书中**直接导入皮肤**，无需编译步骤
- 📱 手机上快速切换，不需要 jsonnet 环境

### 🔄 与上游同步策略

- `main` 分支跟踪上游，保持纯净
- `custom` 分支定期合并 `main`，保留个性化修改
- 冲突时优先保留 custom 的视觉定制

---

## ⚡ 快速开始

1. 在仓/元书中导入本仓库的 `light/` 和 `dark/` 目录
2. 选择「空山素影」皮肤
